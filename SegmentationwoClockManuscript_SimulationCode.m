...
su=1; %% fraction of su effect on ppp %% su=1.2 for treatment, 1 for control
pulse=4; %%pulse freq in each cycle
...
f=1./(1+exp(5*(x-35)/L)); %clock frequency spatial profile
f=2*pi/30*f; %frequency unit conversion
for i=1:T
...
%for parameters = 1:20000 % use here when doing parameter search
 ...
 ppdStep = .02:.02:.2; % 10 steps
   pcdStep = .02:.02:.2; % 10 steps
    ...
    Diff = 2; % Ligand Diffusion Speed=2 (set this to zero for no diffusion hypothesis)
    
    
    ppd  = 0.15; % Dephosphorylation Rate due to Repressor=0.6
    pcd = 0.9; % Dephosphorylation Rate due to Clock=0.3
    ppp = 0.6; % Phosphorylation Rate=0.6
   
    pppm=ppp*ones(1,T); % modified phosphorylation constants for su treatment
    for t=1:T
    ...
    prp = 2.4; % Repressor Protein Translation Rate=1.2
    ...
    mesh(StatMatrix(:,T-380:T,1)) %Tail bud Frame ppERK kymograph
    
    figure
    PPMatrix=zmatrix;
for i=1:size(PPMatrix,2)
PPMatrix(:,i)=PPMatrix(:,i)/max(PPMatrix(1,:)); % normalization
end

for j=T+1:-1:T-380
PPMatrix(1:floor((T+1-j)*Vg),j)=NaN; %empty locations void of PSM cells
end

mesh(PPMatrix(:,T-380:T)) %plot ppERK kymograph
...
mesh(NormMatrix(:,T-380:T)) %plot Normalized ppERK kymograph

%end
figure
%offs=35;
SFC=zmatrix; %start defining SFC matrix
SFC=SFC*0;
for i=1:T+1
    j=1;
    while SFC(j,i)<=1 && j<L-tailbud+1
        j=j+1;
...
SFC(floor((T+1-j)*Vg)+1:floor((T+1-j)*Vg)+tailbud-2,j)=0;
end

mesh(SFC(:,T-380:T)) %plot SFC kymograph

%mesh(SFC)
figure
k=10;
t=2150-k*8;
plot(1:size(SFC,1),SFC(:,t),1:size(SFC,1),SFC(:,t+k),1:size(SFC,1),SFC(:,t+2*k),1:size(SFC,1),SFC(:,t+3*k),1:size(SFC,1),SFC(:,t+4*k),1:size(SFC,1),SFC(:,t+5*k),1:size(SFC,1),SFC(:,t+6*k),1:size(SFC,1),SFC(:,t+7*k),1:size(SFC,1),SFC(:,t+8*k)) %SFC snaphsots with equal intervals showing dicrete shifts
